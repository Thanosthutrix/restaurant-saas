...
import {
  startPlatformPaOAuthAction,
  syncPlatformPaEmittedInvoicesAction,
} from "../paActions";
...
type Props = {
  connection: {
    ...
    lastInvoiceEmittedAt: string | null;
...
export function PlatformEmittedPaCard({ connection }: Props) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-gray-900">
            Émission factures électroniques (Super PDP)
          </p>
          <p className="text-xs text-gray-500">
            ...
            {connection?.companyVerificationStatus === "needs_review"
              ...
              : ""}
            {connection?.lastInvoiceEmittedAt
              ? ` · dernière synchro : ${new Date(connection.lastInvoiceEmittedAt).toLocaleString("fr-FR")}`
              : ""}
          </p>
          <p className="mt-1 text-xs text-gray-400">
            La connexion PA couvre réception et émission (annuaire send + receive).
          </p>
            ...
            <button
              ...
              onClick={() =>
                startTransition(async () => {
                  const res = await syncPlatformPaEmittedInvoicesAction();
                  if (!res.ok) setMessage(res.error);
                  else
                    setMessage(
                      `${res.data!.fetched} lue(s), ${res.data!.updated} mise(s) à jour, ${res.data!.skipped} ignorée(s).`
                    );
                })
              }
              disabled={pending}

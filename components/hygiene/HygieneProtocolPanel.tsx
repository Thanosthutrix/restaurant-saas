...
export function HygieneProtocolPanel({
  ...
  return (
    <div className="mt-3 space-y-2 rounded-lg border border-indigo-100 bg-indigo-50/50 p-3 text-xs text-slate-700">
      <p className="font-semibold text-slate-900">Protocole à suivre</p>
      {description ? <p className="text-slate-600">{description}</p> : null}
      {cleaningProtocol ? (
        <div>
          <p className="font-medium text-slate-800">Nettoyage</p>
          <pre className="mt-0.5 whitespace-pre-wrap font-sans leading-relaxed">{cleaningProtocol}</pre>
        </div>
      ) : null}
      {disinfectionProtocol ? (
        <div>
          <p className="font-medium text-slate-800">Désinfection</p>

  ...
  if (missing) {
    return (
      <div className="space-y-4 text-center">
        <p className="text-sm text-stone-700">Aucune rubrique à valider.</p>
          ...
          rows.map((row, index) => (
            <div key={row.clientId} className={`${uiCard} grid gap-3 sm:grid-cols-[auto_1fr_1fr_auto] sm:items-end`}>
              ...
              <label className="space-y-1">
                <span className="block text-xs font-medium text-stone-500">Plat</span>
                <input
                  value={row.dish_name}
                  onChange={(e) =>
                    updateRow(index, {
              ...
              <label className="space-y-1">
                <span className="block text-xs font-medium text-stone-500">Rubrique</span>
                <input

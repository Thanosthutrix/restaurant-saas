        ...
        <div className="overflow-auto rounded-lg border border-stone-200">
          <table className="w-full border-collapse text-xs">
            <thead className="sticky top-0 z-10 bg-stone-50">
              <tr>
                <th className="sticky left-0 z-20 border-b border-r border-stone-200 bg-stone-50 px-2 py-2 text-left font-semibold text-stone-600">
                  Heure
                </th>
                {columns.map((c) => (
                  <th
                    key={c.id}
                    className="border-b border-r border-stone-200 px-1 py-2 text-center font-semibold text-stone-700"
                    style={{ minWidth: 64 }}
                  >
                    <span
                ...
                <th className="border-b border-stone-200 bg-stone-100 px-2 py-2 text-center font-semibold text-stone-700">
                  Total
                </th>
              </tr>
            </thead>
            <tbody>
              {slots.map((min) => {
                ...
                return (
                  <tr key={min} className={open ? "" : "bg-stone-50/60"}>
                    <td className="sticky left-0 z-10 border-r border-stone-200 bg-white px-2 py-1 font-mono text-[11px] text-stone-500">
                      {fmtSlot(min)}
                    </td>
                    {columns.map((c) => {
                      const on = memberOnAt(c.id, min);
                      return (
                        <td key={c.id} className="border-r border-stone-100 p-0">
                          <div

...
export function FifoStockBlock({
  ...
  return (
    <div className={uiCard}>
      <h2 className="mb-1 text-sm font-semibold text-stone-900">FIFO & valorisation</h2>
      ...
      {summary.openLots.length === 0 ? (
        ...
        <>
          <dl className="mb-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-3">
            <div>
              <dt className="text-stone-500">Qté dans les lots</dt>
              <dd className="font-semibold text-stone-900">
                {summary.qtyInOpenLots} {stockUnit}
              </dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-stone-500">Valeur (coûts connus)</dt>
              <dd className="font-semibold text-stone-900">
          ...
          <ul className="space-y-2 border-t border-stone-100 pt-3">
            {summary.openLots.map((lot) => (
              <li
                key={lot.lotId}
                className="flex flex-wrap items-baseline justify-between gap-2 text-sm text-stone-700"
              >
                <span>

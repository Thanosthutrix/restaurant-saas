    ...
    <div className="mx-auto max-w-5xl space-y-6 px-4 py-6">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        <Link
          ...
          className={
            filter === "all"
              ? "rounded-full bg-stone-900 px-3 py-1 font-medium text-white"
              : "rounded-full border border-stone-200 px-3 py-1 text-stone-700 hover:bg-stone-50"
          }
        >
          Tous
        </Link>
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-stone-500">Aucun relevé enregistré.</p>
      ) : (
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b border-stone-50">
                  ...
                  <td className="px-3 py-2 font-medium text-stone-900">{r.point_name}</td>
                  <td className="px-3 py-2 text-stone-600">
                    {TEMPERATURE_POINT_TYPE_LABEL_FR[r.point_type]}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums text-stone-900">{r.value} °C</td>
                  ...
                  <td className="max-w-[10rem] px-3 py-2 text-xs text-stone-600">
                    {r.recorded_by_display ?? "—"}
                  </td>
                  <td className="max-w-[18rem] px-3 py-2 text-stone-600">
                    ...
                    {r.product_impact && (
                      <p className="text-xs text-stone-500">Produit : {r.product_impact}</p>
                    )}

...
export function InvoiceAnalysisSection({ analysisView, analysis_status, analysis_error }: Props) {
  ...
  return (
    <div className="mb-4 rounded-lg border border-stone-200 bg-stone-50/80 p-4">
      ...
      {hasPayload && (
        <>
          <dl className="mb-4 grid grid-cols-1 gap-2 text-sm sm:grid-cols-2">
            <div>
              <dt className="text-xs text-stone-500">N° facture extrait</dt>
              <dd className="font-medium text-stone-900">{fmt(analysisView!.invoice_number)}</dd>
            </div>
            <div>
              <dt className="text-xs text-stone-500">Date extraite</dt>
              <dd className="font-medium text-stone-900">{fmt(analysisView!.invoice_date)}</dd>
            </div>
            <div>
              <dt className="text-xs text-stone-500">Montant HT extrait</dt>
              <dd className="font-medium text-stone-900">
                {analysisView!.amount_ht != null ? `${fmt(analysisView!.amount_ht)} €` : "—"}
              </dd>
            </div>
            <div>
              <dt className="text-xs text-stone-500">Montant TTC extrait</dt>
              <dd className="font-medium text-stone-900">
          ...
          Object.values(analysisView!.vendor).some((x) => x != null && String(x).trim() !== "") ? (
            <div className="mb-4 rounded border border-stone-200 bg-white px-3 py-2 text-sm">
              <p className="mb-1 text-xs font-medium text-stone-600">Émetteur détecté sur la facture</p>
              <dl className="grid grid-cols-1 gap-1 text-xs sm:grid-cols-2">
                {analysisView!.vendor.legal_name ? (
                  <div>
                    <dt className="text-stone-500">Raison sociale</dt>
                    <dd className="text-stone-900">{fmt(analysisView!.vendor.legal_name)}</dd>
                  </div>
                ) : null}
                {analysisView!.vendor.siret ? (
                  <div>
                    <dt className="text-stone-500">SIRET</dt>
                    <dd className="text-stone-900">{fmt(analysisView!.vendor.siret)}</dd>
                  </div>
                ) : null}
                {analysisView!.vendor.vat_number ? (
                  <div>
                    <dt className="text-stone-500">N° TVA</dt>
                    <dd className="text-stone-900">{fmt(analysisView!.vendor.vat_number)}</dd>
                  </div>
                ) : null}
                {analysisView!.vendor.email ? (
                  <div>
                    <dt className="text-stone-500">E-mail</dt>
                    <dd className="text-stone-900">{fmt(analysisView!.vendor.email)}</dd>
                  </div>
                ) : null}
                {analysisView!.vendor.phone ? (
                  <div>
                    <dt className="text-stone-500">Téléphone</dt>
                    <dd className="text-stone-900">{fmt(analysisView!.vendor.phone)}</dd>
                  </div>
                ) : null}
                {analysisView!.vendor.address ? (
                  <div className="sm:col-span-2">
                    <dt className="text-stone-500">Adresse</dt>
                    <dd className="text-stone-900">{fmt(analysisView!.vendor.address)}</dd>
                  </div>
                ) : null}
              </dl>
          ...
          <p className="mb-2 text-xs text-stone-500">
            Les totaux ligne sont attendus en <strong className="font-medium text-stone-600">HT</strong> pour correspondre au montant HT facture dans le rapprochement.
          </p>
          <h3 className="mb-2 text-xs font-medium text-stone-600">
            Lignes extraites
          </h3>
          {analysisView!.lines.length === 0 ? (
            <p className="text-sm text-stone-600">Aucune ligne structurée dans le résultat d’analyse.</p>
          ) : (
            <div className="overflow-x-auto rounded border border-stone-200">
              <table className="w-full min-w-[36rem] border-collapse text-left text-sm">
                <thead>
                  <tr className="border-b border-stone-200 bg-stone-50">
                    <th className="px-2 py-2 font-medium text-stone-700">Libellé</th>
                    <th className="px-2 py-2 font-medium text-stone-700">Qté</th>
                    <th className="px-2 py-2 font-medium text-stone-700">Unité</th>
                    <th className="px-2 py-2 font-medium text-stone-700">Prix unit.</th>
                    <th className="px-2 py-2 font-medium text-stone-700">Total ligne (HT)</th>
                  </tr>
                </thead>
                <tbody>
                  {analysisView!.lines.map((line, i) => (
                    <tr key={i} className="border-b border-stone-100">
                      <td className="px-2 py-2 text-stone-900">{line.label}</td>
                      <td className="px-2 py-2 text-stone-700">{fmt(line.quantity)}</td>
                      <td className="px-2 py-2 text-stone-700">{fmt(line.unit)}</td>
          ...
          {analysisView!.raw_text && (
            <details className="mt-3 text-sm">
              ...
              <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded border border-stone-200 bg-stone-50 p-2 text-xs text-stone-800">
                {analysisView!.raw_text}
              </pre>
            </details>
          )}
